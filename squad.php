          ...
          <div class="navigation ">
            <nav>
              <ul class="sf-menu">
               ...
               <li  class="current"><a href="spec.php">Группы</a></li>               
...
<section id="content"><div class="ic"></div>
  <div class="container">
    <div class="row">
      <div class="grid_12">
        <h3>Группы (режим преподавателя)</h3>       
      </div>
...
$s="SELECT squad.*, spec, faculty FROM squad, faculty, spec where spec.idspec=squad.idspec and faculty.idfaculty=squad.idfaculty and squad" ;
if ($value1!="Все")
$s=$s." LIKE '%$value1"."%'  ";
else
$s=$s."=squad ";
		 ...
		 $s="SELECT squad.*, spec, faculty FROM squad, faculty, spec where spec.idspec=squad.idspec and faculty.idfaculty=squad.idfaculty ";
...
<input  type="button"  name="button" <? if (mysql_num_rows($r)==0) {?>    disabled="disabled"<? }?>  onclick="this.form.action='updsquad.php?upd=1&step=1'; this.form.submit();" value="Редактировать">   
<input  type="button"  name="button" <? if (mysql_num_rows($r)==0) {?>    disabled="disabled"<? }?>  onclick="qwest=window.confirm('Вы дествительно хотите удалить запись?');  if (qwest) {this.form.action='delsquad.php'; this.form.submit();}" value="Удалить">
<table WIDTH=100% border="1" cellspacing=0 cellpadding=2  >
	...
	if ($i==0)
    echo "<td><input type=radio checked=checked name=Arr[] value=".$f["idsquad"]."> </td>";
	else
    echo "<td><input type=radio name=Arr[] value=".$f["idsquad"]."> </td>";				
				echo "
				<td> $f[squad]</td>								
				<td> $f[faculty]</td>						
				<td> $f[spec]</td>	
