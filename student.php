...
<section id="content"><div class="ic"></div>
  <div class="container">
    <div class="row">
      <div class="grid_12">
        <h3>Студенты (режим преподавателя)</h3>       
      </div>
...
$s="SELECT student.*, squad FROM student, squad where student.idsquad=squad.idsquad ";
...
<input  type="button"  name="button4"   onclick="this.form.action='updstudent.php?upd=0&step=1'; this.form.submit();" value="Добавить">
<input  type="button"  name="button" <? if (mysql_num_rows($r)==0) {?>    disabled="disabled"<? }?>  onclick="this.form.action='updstudent.php?upd=1&step=1'; this.form.submit();" value="Редактировать">   
<input  type="button"  name="button" <? if (mysql_num_rows($r)==0) {?>    disabled="disabled"<? }?>  onclick="qwest=window.confirm('Вы дествительно хотите удалить запись?');  if (qwest) {this.form.action='delstudent.php'; this.form.submit();}" value="Удалить">
...
<table WIDTH=100% border="1" cellspacing=0 cellpadding=2  >
	...
	if ($i==0)
    echo "<td><input type=radio checked=checked name=Arr[] value=".$f["idstudent"]."> </td>";
	else
    echo "<td><input type=radio name=Arr[] value=".$f["idstudent"]."> </td>";				
				echo "
				<td> $f[student]</td>		
				<td> $f[squad]</td>		
				<td> $f[datebirth]</td>						
